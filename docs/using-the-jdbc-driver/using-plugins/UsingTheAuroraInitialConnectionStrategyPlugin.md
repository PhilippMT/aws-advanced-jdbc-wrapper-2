...
|---------------------------------------------------|:-------:|:--------:|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------|---------------|
| ~~`readerInitialConnectionHostSelectorStrategy`~~ | String  |    No    | **Deprecated. Use `initialConnectionHostSelectorStrategy` instead.** <br><br>The strategy that will be used to select a new reader host when opening a new connection. <br><br> For more information on the available reader selection strategies, see this [table](../ReaderSelectionStrategies.md).                                                                                                                | `leastConnections` | `random`      |
| `initialConnectionHostSelectorStrategy`           | String  |    No    | The strategy that will be used to select a host when opening a new connection. A host will be selected according to specified host role. If host role isn't provided then any available host can be selected by the strategy. See `verifyOpenedConnectionType` parameter for details. <br><br> For more information on the available reader selection strategies, see this [table](../ReaderSelectionStrategies.md). | `leastConnections` | `random`      |
| `verifyOpenedConnectionType`                      | String  |    No    | Verify opened connection host role. If parameter isn't set up then **no host role verification** is performed except the following endpoints for which verification is always performed: cluster writer endpoint (`writer`), cluster reader endpoint (`reader`), global database endpoint (`writer`). <br><br>Possible values are `reader` and `writer`.                                                             | `reader`           |               |
